...
const ContainerCodeGame = ({ sendData }: Props) => {
  ...
  return (
    <>
      <section className="relative bg-[#1A1B1F]  flex lg:flex-row flex-col-reverse justify-between rounded-xl mt-4">
        <section className=" px-2 z-50 opacity-0 hover:opacity-100  h-full absolute  flex justify-center items-center flex-col gap-2  lg:h-[20rem] lg:w-[35rem] md:w-[30rem] sm:w-[25rem] w-[20rem]">
          <span className="font-bold text-md  bg-green-700 text-white p-2 rounded-md text-center">
        ...
        <section className=" flex justify-center items-center flex-col gap-2 h-64 lg:h-[20rem] lg:w-[35rem] md:w-[30rem] sm:w-[25rem] w-[20rem]">
          ...
          <div className="mt-5">
            {code.trim().length < 4 ? (
              <button
                disabled
                className=" -z-10 bg-[#B6B6B6] text-black px-4 py-1 rounded-lg cursor-not-allowed"
              >
                Entrar no jogo
              </button>
            ) : (
              <button
                ...
                className=" -z-10 bg-[#3D4EFB] px-4 py-1 rounded-lg hover:scale-105 transition-all ease-linear duration-200"
              >
                Entrar no jogo
              </button>
            )}
